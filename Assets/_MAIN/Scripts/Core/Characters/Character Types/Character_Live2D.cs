...
namespace CHARACTERS
{
    public class Character_Live2D : Character
    {
        ...
        {
            float targerAlpha = show ? 1f : 0f;

            while (renderController.Opacity != targerAlpha)
            {
                renderController.Opacity = Mathf.MoveTowards(renderController.Opacity, targerAlpha, DEFAULT_TRANSITION_SPEED * Time.deltaTime * speedMultiplier);
                yield return null;
            }
