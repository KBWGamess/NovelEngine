...
namespace DIALOGUE
{
    ...
    {
        private const int DEAFULT_CHARACTERS_READ_PER_SECOND = 18;
        ...
        private IEnumerator AutoRead()
        {
            ...
            while (conversationManager.isRunning)
            {
                if (!skip)
                {
                    ...
                    float timeToRead = Mathf.Clamp(((float)architect.tmpro.textInfo.characterCount / DEAFULT_CHARACTERS_READ_PER_SECOND), MIN_READ_TIME, MAX_READ_TIME);
