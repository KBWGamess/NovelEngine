...
namespace DIALOGUE
{
    public class DL_SPEAKER_DATA
    {
        ...
        private const string POSITTIONCAST_ID = " at ";
        ...
        public DL_SPEAKER_DATA(string rawSpeaker)
        {
            ...
            string pattern = @$"{NAMECAST_ID}|{POSITTIONCAST_ID}|{EXPRESSIONCAST_ID.Insert(EXPRESSIONCAST_ID.Length - 1, @"\")}";
            ...
            for (int i = 0; i < matches.Count; i++)
            {
                ...
                else if (match.Value == POSITTIONCAST_ID)
                {
                    isCastingPosition = true;

                    startIndex = match.Index + POSITTIONCAST_ID.Length;
                    endIndex = i < matches.Count - 1 ? matches[i + 1].Index : rawSpeaker.Length;
